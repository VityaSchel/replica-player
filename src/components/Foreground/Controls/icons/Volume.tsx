...
export default function VolumeMute(props: { 
  ...
  React.useEffect(() => {
    const from2to1 = volume >= 0.5 && props.volume < 0.5
    const from1to2 = volume < 0.5 && props.volume >= 0.5
    // const fromUnmutetoMute = volume !== 0 && props.volume === 0
    // const fromMuteToUnmute = volume === 0 && props.volume !== 0

    from2to1 && setIconState('from2to1')
    from1to2 && setIconState('from1to2')
    // from1toMute && setIconState('from1toMute')
    // fromMuteTo1 && setIconState('fromMuteTo1')
    // fromUnmutetoMute && setIconState('fromUnmuteToMute')
    // fromMuteToUnmute && setIconState('fromMuteToUnmute')
